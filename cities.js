...
function averagePopulation(data) {
  const pops = _.pluck(data, 'population');
  const theAverage = _.reduce(pops, function (memo, num) { return memo + num; }, 0) / data.length;
  return theAverage;
}

function biggestAndSmallest(data) {
  const pops = _.pluck(data, 'population');
  const max = _.max(pops, function (entry) {
    return entry;
  });
  const biggest = _.filter(data, function (entry) {
    return entry.population === max;
  });
  const min = _.min(pops, function (entry) {
    return entry;
  });
  const smallest = _.filter(data, function (entry) {
    return entry.population === min;
  });
